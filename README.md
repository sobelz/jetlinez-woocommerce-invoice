...
برای جلوگیری از ارسال تکراری، هر اجرای خودکار روزانه پیش از ارسال یک شناسهٔ یکتای ماندگار در دیتابیس ثبت می‌کند. اگر همان نوبت Cron دوباره یا هم‌زمان اجرا شود، اجرای دوم بدون ارسال پیام متوقف و رویداد `Duplicate daily report cron execution skipped` در لاگ WooCommerce با منبع `jetlinez-invoice` ثبت می‌شود. این محافظ روی دکمهٔ ارسال فوری اثری ندارد.

برای مشاهدهٔ همه رویدادهای ثبت‌شدهٔ گزارش روزانه با WP-CLI می‌توان فرمان زیر را اجرا کرد؛ وجود بیش از یک ردیف، ثبت چند رویداد را نشان می‌دهد:

```bash
wp cron event list --hook=jlwi_send_daily_report
```

ذخیرهٔ دوبارهٔ تب گزارش‌ها همه رویدادهای همین hook را پاک می‌کند و فقط نوبت بعدی را می‌سازد. لاگ‌های شروع، پایان، خطا و جلوگیری از اجرای تکراری از مسیر **WooCommerce → Status → Logs** و با انتخاب منبع `jetlinez-invoice` قابل بررسی هستند.
